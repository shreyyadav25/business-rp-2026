...
# ============================================
# MONTE CARLO SIMULATION (COLAB-SAFE VERSION)
# ============================================
...
# -------------------------------
TRUE_BETA_AI = 2.8
TRUE_BETA_VERIF = -0.62
NOISE_SD = 4.2

N_FIRMS = 300          # Reduced for tractability
YEARS = [2022, 2023, 2024, 2025]
N_REPS = 50            # Monte Carlo repetitions
...
# -------------------------------
def generate_panel(seed, endog_ai=False, endog_verif=False, meas_error=False):
    np.random.seed(seed)
    rows = []

    for f in range(N_FIRMS):
        firm_id = f"FIRM_{f:03d}"
        firm_shock = np.random.normal(0, 1) if endog_ai else 0

        for yr in YEARS:
            ai_index = np.random.beta(2 + (yr - 2022), 3)

            if endog_ai:
                ai_index += 0.40 * firm_shock
                ai_index = np.clip(ai_index, 0, 1)

            verification = np.random.binomial(1, 0.57)
            if endog_verif:
                verification = int(firm_shock > 0)

            ai_x_verif = ai_index * verification

            ceo_span_control = (
                12.4
                + TRUE_BETA_AI * ai_index
                + TRUE_BETA_VERIF * ai_x_verif
                + np.random.normal(0, NOISE_SD)
            )

            if meas_error:
                ai_index += np.random.normal(0, 0.10)

            rows.append({
                "firm_id": firm_id,
                "year": yr,
                "ai_index": ai_index,
                "ai_x_verif": ai_x_verif,
                "ceo_span_control": ceo_span_control
            })

    df = pd.DataFrame(rows)
    assert "ceo_span_control" in df.columns
    return df
...
def run_regression(df):
    model = smf.ols(
        "ceo_span_control ~ ai_index + ai_x_verif + C(firm_id) + C(year)",
        data=df
    ).fit()
    return model.params["ai_index"]
...
scenarios = {
    "baseline": dict(endog_ai=False, endog_verif=False, meas_error=False),
    "endogenous_ai": dict(endog_ai=True, endog_verif=False, meas_error=False),
    "endogenous_verification": dict(endog_ai=False, endog_verif=True, meas_error=False),
    "measurement_error": dict(endog_ai=False, endog_verif=False, meas_error=True)
}
...
for name, params in scenarios.items():
    ...
    for r in range(N_REPS):
        df = generate_panel(seed=1000 + r, **params)
        coef = run_regression(df)
    ...
    results.append({
        "scenario": name,
        "mean_ai_coef": round(estimates.mean(), 3),
        "bias": round(estimates.mean() - TRUE_BETA_AI, 3),
        "std_dev": round(estimates.std(), 3)
    })
